...
type = "ar_0.5"
fileloc <- paste0("/section-3/MLE-single/output/", type , "/")
...
Sigma <- as.matrix(read.table(
  paste0("/section-3/MLE-single/param/", type, "/Sigma.txt")))
non_null <- scan(paste0(dir, "/section-3/MLE-single/param/", type, "/non_null.txt"))
...
j_nonnull <- scan(paste0(dir,"/section-3/MLE-single/param/", type, "/j_nonnull.txt"))
# randomly pick a non-null
j <- 2
beta <- beta[non_null[j]]
...
# adjusted beta_hat
beta_adj <- (beta_hat[,j] - alpha_s * beta) * tau[j] / sigma_s
...
filename <- "/section-3/MLE-single/Figure/sec2_3.png"
