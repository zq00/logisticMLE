...
#Table 3 : t-test
tilde_beta_c <- beta_est/std_est # classical z-statistics
phi = 2-2*pnorm(abs(tilde_beta_c[,jnull]), 0, 1, lower.tail=TRUE)
...
adj_c <- 1
p.val=data.frame(pchisq(dev[1:N]/adj_c, lower.tail=FALSE, df=1))
phi <- p.val
